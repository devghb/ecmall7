...
class ECMall
{
    ...
    static function startup($config = array())
    {
        ...
        /* 请求转发 */
        $default_module = $config['default_module'] ? $config['default_module'] : 'default';
        $default_act = $config['default_act'] ? $config['default_act'] : 'index';

        $module    = isset($_REQUEST['module']) ? trim($_REQUEST['module']) : $default_module;
        $act    = isset($_REQUEST['act']) ? trim($_REQUEST['act']) : $default_act;

        define('MODULE', $module);
        define('ACT', $act);

        $module_class_file = ROOT_PATH . '/external/modules/' . $module . '/index.module.php';
...
function &db()
{
    include_once(ROOT_PATH . '/eccore/model/pdo.php');
    static $db = null;
    if ($db === null)
    {
        $db = new cls_pdo(array(
            'type'  => DB_TYPE,
            'host'  => DB_HOST,
            'port'  => DB_PORT,
            'name'  => DB_NAME,
