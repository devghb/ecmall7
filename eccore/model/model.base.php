...
class BaseModel extends Object
{
    ...
    function find($params = array())
    {
        ...
        return $index_key ? $this->db->getAllWithIndex($sql, $index_key) :  $this->db->getAll($sql);
    }
    ...
    function add($data, $compatible = false)
    {
        ...
        $this->db->query("{$mode} INTO {$this->table}{$insert_info['fields']} VALUES{$insert_info['values']}",$insert_info['execute']);
    ...
    function edit($conditions, $edit_data)
    {
        ...
        $this->db->query("UPDATE {$this->table} SET {$edit_fields['fields']}{$conditions}",$edit_fields['execute']);
    ...
    function getRealFields($src_fields_list)
    {
        ...
        //$fields = preg_replace('/([a-zA-Z0-9_]+)\.([a-zA-Z0-9_*]+)/', "\$this->_getFieldTable('\\1') . '.\\2'", $fields);
        $fields = preg_replace_callback('/([a-zA-Z0-9_]+)\.([a-zA-Z0-9_*]+)/',function($r){
    ...
    function _getInsertInfo($data)
    {
        ...
        return compact('fields', 'values', 'length','execute');
    }
